...
import com.jkys.consult.referTo.bean.advisory.StoreAdvisory;
...
//    private MemberAdvisoryRpcService memberAdvisoryRpcService;
    ...
    public void finishAdvisoryExpire48(PatientAdvisoryInfo info) {

        //todo 超过20191101号删除
//        specialAdvisoryService.delKey(info.getDoctorId(),info.getPatientId());
...
//            kafkaServer.sendMsg(info);
            return;
        }

        //医生无回复的需要退云币
        if (order.getStatus() == OrderStatus.PAY_SUCCESS.getCode()) {
            Map<String, Object> patientMaps= userCenterUserService.findExtraUserById(info.getPatientId());
            boolean isSend =true;
            if(patientMaps !=null && !patientMaps.isEmpty() && patientMaps.get("accId")!=null){
                isSend = !patientMaps.get("accId").toString().equals("7health");
            }
    ...
    public void cancelAdvisory(PatientAdvisoryInfo info) {
        ...
        log.warn("咨询任务：取消订单 订单状态错误：patient:{},number:{},status:{}", info.getPatientId(), info.getOrderNum(),info.getStatus());
    }
...
//                if (StringUtils.isNotEmpty(baseUserInfo.getMobile()) && ContextUtil.isProduction()) {
...
//                }
    ...
    boolean isPaySuccess(String orderNum) {
        ...
        if (response != null && response.getSuccess() && response.getPayList() != null && !response.getPayList().isEmpty()) {
...
//            throw new ShineServiceException(ExceptionCode.ASK_PAY_FAIL);
        }
        return response;
    }
...
/*        newOrder = patientAdvisoryOrderMapper.findByOrderAndType(order.getOrderNum(), OrderType.BACK.name());
    ...
    /*生成咨询订单*/
    PatientAdvisoryOrder initOrder(Long doctorId, Long patientId, String orderNum, Integer status, String type) {
        PatientAdvisoryOrder order = new PatientAdvisoryOrder();
        order.setStatus(status);
        order.setDoctorId(doctorId);
        order.setPatientId(patientId);
        if (!type.equals(OrderType.FREE.name())) {
            Integer price = doctorPrice(doctorId);
...
//        return info;
//    }
Integer doctorPrice(Long doctorId) {
...
/*
    ...
    Integer doctorOrigin(Long doctorId) {
        Integer price = doctorAdvisoryPriceMapper.queryDoctorAdvisoryPrice(doctorId);
...
//    }
    ...
    private String genBizCode(String type) {
        return type + DateUtil.format(new Date(), DateUtil.PATTERN_YEAR2DAY) + StringUtils
            .leftPad(sequenceGenerator.get() + "", 11, '0');
    }
    ...
    public String genOrderNum(String type) {
        synchronized (lock) {
            ...
            return type + System.currentTimeMillis() + StringUtils
                .leftPad(Integer.toString(num), 5, '0');
        }
    }
