...
public interface PatientAdvisoryInfoMapper {
    ...
    @Select("<script>select count(1) from patient_advisory_info" + " where 1=1 and status >0" +
            "<if test=\"query.doctorIds !=null \">and doctor_id  " +
            "in <foreach close=\")\" collection=\"query.doctorIds\" item=\"id\" open=\"(\" separator=\",\"> #{id} " +
            "</foreach> </if> " +
            "<if test=\"query.patientIds !=null \">and patient_id  " +
            "in <foreach close=\")\" collection=\"query.patientIds\" item=\"id\" open=\"(\" separator=\",\"> #{id} " +
            "</foreach> </if> " +
            ...
            " </script>")
    Long countUserInfoList111(@Param("query") AdvisoryQuery query);
     ...
     * @return result
     */
    @Select("select id,doctor_id,patient_id,order_num,start_time,status from patient_advisory_info where  status =1 and source in('IN_APP','MEMBER') and gmt_create <#{days} ")
...
//     * @return result
//     */
//    @Select("select id,doctor_id,patient_id,order_num,start_time,status from patient_advisory_info where status in(1,2) and order_num like '%PAY%' and start_time is null and describes is null and gmt_create <'2018-09-14 14:41:57' ")
//    List<PatientAdvisoryInfo> findRefundAndCancelInfoList();
